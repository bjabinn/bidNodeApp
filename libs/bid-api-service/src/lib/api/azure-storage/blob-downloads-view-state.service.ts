...
import { BlobContainerRequest, BlobItemDownload } from '@bid/bid-api-service';
    ...
    private mapDownloadResponse = (
        ...
        source.pipe(
            map(url => ({
                ...
                url: options.storageUri + options.containerName + '/' + filename
            })),
            startWith({
                filename,
                containerName: options.containerName,
        ...
        source.pipe(
            switchMap(res =>
                from(res.blobBody).pipe(
                    ...
                    map(
                        url =>
                            this.sanitizer.bypassSecurityTrustUrl(url) as string
                    )
                )
            )
        );
